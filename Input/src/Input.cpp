...
static constexpr auto end_input   = std::to_underlying(MouseButton::_END);
...
void IInput::AddAction(size_t pos, const std::function<void(IInput*)>& action)
{
	if (pos < GetOffset(Button::_END))
	{
		return AddGamepadAction(static_cast<Button>( pos ), action);
	}
	if (pos < GetOffset(Key::_END))
	{
		return AddKeyboardAction(static_cast<Key>( pos ), action);
	}
	if (pos < GetOffset(MouseButton::_END))
	{
		return AddMouseAction(static_cast<MouseButton>( pos ), action);
	}
}
...
bool IInput::IsDown(size_t pos) const
{
	if (pos < GetOffset(Button::_END))
	{
		return GetButton(static_cast<Button>( pos ));
	}
	if (pos < GetOffset(Key::_END))
	{
		return GetKey(static_cast<Key>( pos ));
	}
	if (pos < GetOffset(MouseButton::_END))
	{
		return GetMouseButton(static_cast<MouseButton>( pos ));
	}
	return false;
}
...
const char* GetMouseButtonName(MouseButton m)
{
	switch (m)
	{
	case Input::MouseButton::NONE:	 return "NONE";
	case Input::MouseButton::LEFT:	 return "LEFT";
	case Input::MouseButton::MIDDLE: return "MIDDLE";
	case Input::MouseButton::RIGHT:	 return "RIGHT";
	case Input::MouseButton::AUX_1:	 return "AUX_1";
	case Input::MouseButton::AUX_2:	 return "AUX_2";
	default:                         return "INVALID";
	};
}
const char* GetName(size_t pos)
{
	if (pos < GetOffset(Button::_END))
	{
		return GetButtonName(static_cast<Button>( pos ));
	}
	if (pos < GetOffset(Key::_END))
	{
		return GetKeyName(static_cast<Key>( pos ));
	}
	if (pos < GetOffset(MouseButton::_END))
	{
		return GetMouseButtonName(static_cast<MouseButton>( pos ));
	}
	return "INVALID";
}
...
std::string GetRepr(size_t pos)
{
	if (pos < GetOffset(Button::_END))
	{
		return GetButtonRepr(static_cast<Button>( pos ));
	}
	if (pos < GetOffset(Key::_END))
	{
		return GetKeyRepr(static_cast<Key>( pos ));
	}
	if (pos < GetOffset(MouseButton::_END))
	{
		return GetMouseButtonRepr(static_cast<MouseButton>( pos ));
	}
	return "INVALID";
}
